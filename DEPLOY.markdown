...
This section summarises Tally Server configuration. For more details, see
...
it generates keys:
    ...
    privcount config.yaml ts 2>&1 | tee -a ts.log
...
This section summarises Share Keeper configuration. For more details, see
the subsections below.
...
Run PrivCount in share keeper mode so that it generates keys:
    ...
    privcount config.yaml sk 2>&1 | tee -a sk.log
...
This section summarises Data Collector configuration. For more details, see
the subsections below.
...
Then run PrivCount in data collector mode:
    ...
    privcount config.yaml dc 2>&1 | tee -a dc.log
